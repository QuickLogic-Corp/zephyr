...
#define CLKOUT_INIT(N)							       \
	...
	if (lcko->id > ldev->nclkout) {					       \
		LOG_ERR("CLKOUT%d: Invalid CLKOUT index!", lcko->id);	       \
		return -EINVAL;						       \
	}								       \
									       \
	...
	if (lcko->def.duty.den <= 0 ||					       \
		...
		LOG_ERR("CLKOUT%d: Invalid default duty! %d/%d",	       \
			lcko->id, lcko->def.duty.num, lcko->def.duty.den);     \
		return -EINVAL;						       \
	}
...
struct litex_clk_clkout_margin {
	u32_t m;			/* margin factor scaled to integer */
	u32_t exp;
};
...
struct litex_clk_device {
	...
	u8_t *update_clkout;			/* which clkout needs update */
	u32_t sys_clk_freq;			/* input frequency */
	u32_t vco_margin;
	u32_t nclkout;
};
