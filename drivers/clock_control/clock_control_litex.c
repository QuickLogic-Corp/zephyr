 ...
 * https://github.com/torvalds/linux/commit/448c3c057a1dbb6ecf6e507a3f1a58f5eab21560
 * file path: drivers/clk/clk-axi-clkgen.c
 *
...
static u32_t litex_clk_lookup_filter(u32_t glob_mul)
{
	switch (glob_mul) {
	case 0:
		return 0x01001990;
...
static const u32_t litex_clk_lock_table[] = {
...
static u32_t litex_clk_lookup_lock(u32_t glob_mul)
{
...
static inline void litex_clk_set_reg(u32_t reg, u32_t val)
{
	litex_write((u32_t *)drp[reg].addr, drp[reg].size, val);
}

static inline u32_t litex_clk_get_reg(u32_t reg)
{
	return litex_read((u32_t *)drp[reg].addr, drp[reg].size);
}

static inline void litex_clk_assert_reg(u32_t reg)
{
...
static inline void litex_clk_deassert_reg(u32_t reg)
{
	litex_clk_set_reg(reg, ZERO_REG);
}

static int litex_clk_wait(u32_t reg)
{
	u32_t timeout;

	__ASSERT(reg == DRP_LOCKED || reg == DRP_DRDY, "Wrong reg value!");
...
static int litex_clk_get_DO(u8_t clk_reg_addr, u16_t *res)
{
	int ret;
...
static u64_t litex_clk_calc_global_frequency(u32_t mul, u32_t div)
{
	u64_t f;

	f = (u64_t)ldev->sys_clk_freq * (u64_t)mul;
	f /= div;
...
static u64_t litex_clk_get_real_global_frequency(void)
{
	u64_t f;
...
static void litex_clk_check_DO(char *reg_name, u8_t clk_reg_addr, u16_t *res)
{
	int ret;
...
{
	u16_t power_reg, div_reg, clkfbout_reg1, clkfbout_reg2,
...
static void litex_clk_print_clkout_regs(u8_t clkout, u8_t reg1, u8_t reg2)
{
	u16_t clkout_reg1, clkout_reg2;
	char reg_name[16];
...
static inline u16_t litex_clk_calc_DI(u16_t DO_val, u16_t mask, u16_t bitset)
{
	u16_t DI_val;
...
static int litex_clk_set_DI(u16_t DI_val)
{
	int ret;
 ...
 *
 */
static int litex_clk_change_value(u16_t mask, u16_t bitset, u8_t clk_reg_addr)
{
	u16_t DO_val, DI_val;
	int ret;
 ...
 *
 */
static int litex_clk_set_clock(u8_t clkout_nr, u16_t mask_reg1,
			      u16_t bitset_reg1, u16_t mask_reg2,
						 u16_t bitset_reg2)
{
	struct litex_clk_regs_addr drp_addr = litex_clk_regs_addr_init();
	int ret;
...
{
	int ret;
	u8_t no_cnt = 0, edge = 0, ht = 0, lt = 0, div = ldev->ts_g_config.div;
	u16_t bitset = 0;
...
{
	int ret;
	u8_t no_cnt = 0, edge = 0, ht = 0, lt = 0, mul = ldev->ts_g_config.mul;
	u16_t bitset1 = 0;
...
static int litex_clk_set_filt(void)
{
	...
	ret = litex_clk_change_value(FILT_MASK, filt >> 16, FILT_REG1);
	if (ret != 0)  {
		return ret;
	}
	ret = litex_clk_change_value(FILT_MASK, filt, FILT_REG2);
	return ret;
}
...
static int litex_clk_set_lock(void)
{
	...
	ret = litex_clk_change_value(LOCK1_MASK, lock & 0x3ff, LOCK_REG1);
	if (ret != 0) {
		return ret;
	}
	ret = litex_clk_change_value(LOCK23_MASK,
...
/* Round scaled value*/
static inline u32_t litex_round(u32_t val, u32_t mod)
{
	if (val % mod > mod / 2) {
		return val / mod + 1;
	}
			     ...
			     struct clk_duty *duty)
{
	...
	u32_t divider;
	u16_t clkout_reg1, clkout_reg2;
	u8_t clkout_nr, high_time, edge, no_cnt, frac_en, frac_cnt;
...
static inline u8_t litex_clk_calc_duty_percent(struct clk_duty *duty)
{
	u32_t div, duty_ratio, ht;

	ht = duty->num;
	div = duty->den;
	duty_ratio = ht * 10000 / div;

	return (u8_t)litex_round(duty_ratio, 100);
}
					 ...
					 int calc_new)
{
	struct clk_duty duty;
	int delta_d;
	u32_t ht_aprox, synthetic_duty, min_d;
	u8_t high_time_it, edge_it, high_duty,
	...
	for (high_time_it = 1; high_time_it < divider; high_time_it++) {
		for (edge_it = 0; edge_it < 2; edge_it++) {
			synthetic_duty = (high_time_it * 100 + 50 * edge_it) /
								    divider;
			delta_d = synthetic_duty - high_duty;
			delta_d = abs(delta_d);
...
int litex_clk_set_duty_cycle(struct litex_clk_clkout *lcko,
			     struct clk_duty *duty)
{
	int ret;
	u16_t bitset1, bitset2;
	u8_t clkout_nr = lcko->id,
	   *edge = &lcko->div.edge,
	...
	if (lcko->frac.frac == 0) {
		...
		if (ret != 0) {
			LOG_ERR("CLKOUT%d: cannot set %d%% duty cycle for that frequency",
				clkout_nr, high_duty);
			return ret;
		}
...
{
	u64_t period_buff;
	u32_t post_glob_div_f, global_period,  clkout_period,
	    *period_off = &lcko->ts_config.period_off;
	u8_t divider = lcko->config.div;
	/* ps unit */

	post_glob_div_f = (u32_t)litex_clk_get_real_global_frequency();
	period_buff = PICOS_IN_SEC;
	period_buff /= post_glob_div_f;
	global_period = (u32_t)period_buff;
	clkout_period = global_period * divider;

	if (lcko->ts_config.phase != 0) {
		int synthetic_phase, delta_p, min_p, p_o;
		u8_t d_t, p_m;
...
static int litex_clk_get_phase_data(struct litex_clk_clkout *lcko,
				    u8_t *phase_mux, u8_t *delay_time)
{
	struct litex_clk_regs_addr drp_addr = litex_clk_regs_addr_init();
	int ret;
	u16_t r1, r2;
...
{
	u64_t period_buff;
	u32_t divider = 0, fract_cnt, post_glob_div_f,
	    pm, global_period, clkout_period, period;
	u8_t phase_mux = 0, delay_time = 0;
...
{
	u64_t post_glob_div_f, buff, clkout_period;

	post_glob_div_f = (u32_t)litex_clk_get_real_global_frequency();
	buff = PICOS_IN_SEC;
	buff /= post_glob_div_f;
	clkout_period = (u32_t)buff;
...
{
	int ret;
	u16_t bitset1, bitset2, reg2_mask;
	u8_t *phase_mux = &lcko->phase.phase_mux,
	...
	if (ret != 0) {
		LOG_ERR("CLKOUT%d: phase offset %d deg is too high for given frequency",
			 clkout_nr, degrees);
		return ret;
	}
...
/* Returns rate in Hz */
static inline u32_t litex_clk_calc_rate(struct litex_clk_clkout *lcko)
{
	u64_t f = litex_clk_calc_global_frequency(ldev->ts_g_config.mul,
						  ldev->ts_g_config.div);
 ...
 *
 */
static u32_t litex_clk_pow(u32_t base, u32_t exp)
{
	int ret = 1;
					 ...
					 u64_t vco_freq)
{
	...
	for (d = lcko->clkout_div.min; d <= lcko->clkout_div.max; d++) {
		...
		delta_f = abs(delta_f);
		if (delta_f <= m) {
			lcko->config.freq = (u32_t)clk_freq;
...
static int litex_clk_calc_all_clkout_params(u64_t vco_freq)
{
...
static int litex_clk_calc_all_params(void)
{
	...
	for (div = ldev->divclk.min; div <= ldev->divclk.max; div++) {
		...
		for (mul = ldev->clkfbout.max; mul >= ldev->clkfbout.min; mul--) {
			...
			vco_freq = (u64_t)ldev->sys_clk_freq * (u64_t)mul;
			vco_freq /= div;
...
int litex_clk_check_rate_range(struct litex_clk_clkout *lcko, u32_t rate)
{
	u64_t max, min, m;
	u32_t div, margin;
	...
	max = (u64_t)ldev->sys_clk_freq * (u64_t)ldev->clkfbout.max;
	...
	if ((u64_t)rate < min || (u64_t)rate > max) {
		return -EINVAL;
	}
	return 0;
}
 ...
 */
static int litex_clk_get_subsys_rate(struct device *clock,
				     clock_control_subsys_t sys, u32_t *rate)
{
...
static enum clock_control_status litex_clk_get_status(struct device *dev,
...
static inline int litex_clk_on(struct device *dev, clock_control_subsys_t sys)
{
	struct litex_clk_setup *setup = sys;
	struct clk_duty duty;
	struct litex_clk_clkout *lcko;
...
static inline int litex_clk_off(struct device *dev,
...
static int litex_clk_dts_timeout_read(struct litex_clk_timeout *timeout)
{
	...
	if (timeout->lock < 1) {
		LOG_ERR("LiteX CLK driver cannot wait for time bellow ca. 1ms\n");
		return -EINVAL;
	}
...
static int litex_clk_dts_global_read(void)
{
	...
	ldev->update_clkout = k_malloc(sizeof(u8_t) * ldev->nclkout);
...
static int litex_clk_init(struct device *dev)
{
	int ret;
